...
const {DEVERR, PKG, ENV, WORKSPACES, JSON_EXT, WS_MAP_KEY, WS_IGNORE_KEY} = require('../constants')
...
const findRoot = () => {
    ...
    while (root !== '/') {
        ...
        if (existsSync(pkgPth)) {
            ...
            if (wsPkg[WORKSPACES]) return [wsPkg, root]
        }
        root = resolve(root, '../')
    }
...
const findPackage = pkgName => {
    ...
    if(root === wsRoot) return [wsPkg, wsRoot]
 ...
 * @param {String} root
 */
const mapPackages = (pkg, root) => pkg[WORKSPACES].reduce((acc, ws) => {
    if (ws[ws.length - 1] === '*')
        glob.sync('*/', {
...
const pathFromObj = (key, obj, pth='') => {
    ...
    if (isEmpty(obj)) return pth
    if (!key) return obj.root

    for (let k of Object.keys(obj)) {
        if (k === key) return join(pth, key)
        if (typeof obj[k] === 'object') {
            const _pth = pathFromObj(key, obj[k], join(pth, k))
            if(_pth !== k) return _pth
        }
    }
